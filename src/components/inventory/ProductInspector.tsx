...
export default function ProductInspector({
  ...
  // Fonction de gestion du fetch IA
  const handleUnifiedAIFetch = async (mode: AIFetchMode) => {
    try {
      ...
      if (mode === 'images' || mode === 'all') {
        setAiProgress({ mode, step: 'fetching_images', message: 'Récupération des images...' });
    ...
    if (!imagesResponse.ok) {
        ...
        // Notifier le parent que les images ont changé
        if (imagesData.supabaseImages && imagesData.supabaseImages.length > 0) {
          alert(`✅ ${imagesData.supabaseImages.length} image(s) récupérée(s) et uploadée(s) !`);
